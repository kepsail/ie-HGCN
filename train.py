...
def train(epoch):
	...
	# class_weight = 1 / (label['m'][0].sum(0)).type_as(x_train)
	# class_weight = (label['m'][0].sum(0).type_as(x_train)) / (label['m'][0].sum().type_as(x_train))
	class_weight = (label['m'][0].sum().type_as(x_train)) / (label['m'][0].sum(0).type_as(x_train))
	...
	# idx_train = label['m'][1]
	# x_train = F.log_softmax(output['m'], dim=1)[idx_train]
	# y_train = label['m'][0][idx_train]
	...
	# idx_val = label['m'][2]
	# x_val = F.log_softmax(output['m'], dim=1)[idx_val]
	# y_val = label['m'][0][idx_val]
...
def test():
	...
	# idx_test = label['m'][3]
	# x_test = F.log_softmax(output['m'], dim=1)[idx_test]
	# y_test = label['m'][0][idx_test]
